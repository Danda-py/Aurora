...
Questo è il **sito web indipendente dedicato all'host** per la gestione autonoma degli accessi, delle prenotazioni Bed-and-Breakfast.it, del relè/serratura **Sonoff (eWeLink)** e dei contenuti di Aurora in Valtellina (Morbegno).
...
2. **Integrazione Sonoff & eWeLink**:
   - Gestione nativa del relè apriporta Sonoff (Mini, Basic, SV, 4CH) tramite Home Assistant.
...
3. **Connessione REST API**: Si collega direttamente al backend di Aurora in Valtellina per:
   ...
   - Generare link univoci temporanei (`POST /api/webhook/booking`)
...
5. L’ospite userà il link personale per accedere e aprire la porta.
