...
Questo è il **sito web indipendente dedicato all'host** per la gestione autonoma degli accessi, prenotazioni Bed-and-Breakfast.it, relè/serratura **Sonoff (eWeLink)** e tastierino numerico di Aurora in Valtellina (Morbegno).
...
2. **Integrazione Sonoff & eWeLink**:
   - Gestione nativa del relè apriporta Sonoff (Mini, Basic, SV, 4CH) o tastierino eWeLink.
...
3. **Connessione REST API**: Si collega direttamente al backend di Aurora in Valtellina per:
   ...
   - Generare link univoci con codice PIN casuale a 4 cifre (`POST /api/webhook/booking`)
   - Sincronizzare il tastierino elettronico (`POST /api/keypad/sync`)
...
5. **Condivisione Istantanea**:
   - Invia link univoco su WhatsApp con messaggio precompilato e codice PIN
...
5. **Se hai un tastierino abbinato**: l'ospite userà semplicemente il codice PIN a 4 cifre generato dal link di Aurora.
...
2. Incollalo nella scheda *Sonoff & eWeLink / Tastierino* del Portale Host e clicca su *Salva Configurazione Sonoff*.
